          ...
          {tariffs.map((tariff, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <h3 className={`tariff-title premium-display text-[1.32rem] font-semibold ${tariff.featured ? 'text-[#dce84f]' : 'text-[#f4f1e6]'}`}>{tariff.title}</h3>
                {tariff.featured && <span className="premium-chip rounded-full border border-[#a6b229]/42 bg-[linear-gradient(180deg,rgba(113,121,29,0.44),rgba(84,90,25,0.22))] px-3.5 py-1 text-[0.62rem] font-medium uppercase tracking-[0.24em] text-[#eef6a5] shadow-[0_8px_24px_rgba(182,194,72,0.12),inset_0_1px_0_rgba(255,255,255,0.07)]">выбор клуба</span>}
              </div>
              ...
              <ul className="tariff-list premium-body mt-5 space-y-2.5 text-[0.92rem] font-light leading-[1.72] text-soft/84">
                {tariff.perks.map((perk) => (
                  <li key={perk} className="flex items-start gap-2">
                    <span className="mt-1 inline-block h-2 w-2 rounded-full bg-[linear-gradient(180deg,rgba(223,236,75,0.98),rgba(182,197,48,0.96))] shadow-[0_0_0_3px_rgba(137,149,36,0.18),0_0_14px_rgba(201,216,76,0.16)]" />
                    <span>{perk}</span>
                  </li>
                ))}
              </ul>
